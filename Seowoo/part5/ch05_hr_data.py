...
Original file is located at
    https://colab.research.google.com/drive/1nQAGLoG9rOEAJrcqiMNrV_RwwY89tNhf
...
attrition = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/study/WA_Fn-UseC_-HR-Employee-Attrition.csv')
attrition.head()
...
##### 데이터가 연속성이 없다면 다른 의미로의 수치적 데이터로 변환 시켜주는 것
"""
...
"""##### Feature Skewness 확인
##### 비대칭도(왜도) : 실수 값 확률 변수의 확률 분포 비대칭성을 나타내는 지표. 
##### 비대칭도 < 0 : 확률밀도함수의 왼쪽 부분의 꼬리가 길다.
##### 비대칭도 > 0 : 확률 밀도함수의 오른쪽 부분의 꼬리가 길다.
"""
...
##### GridSearchCV : 모델의 선응향상을 위해 사용되는 기법중 하나.                사용자가 직접 모델의 하이퍼 파라미터 값을     리스트로 입력하면 값에 대한 경우의 수마다 예측 성능을 측정 및 평가하여 최적의 파라미터 값을 찾는 과정. 굵은 텍스트
"""
