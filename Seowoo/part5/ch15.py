...
display(col_info)
temp = pd.DataFrame(df_init.isnull().sum()).T.rename(index={0: 'null values (nb)'})
...
# col_info = pd.concat(col_info, temp)
# temp = pd.DataFrame(df_init.isnull().sum() / df_init.shape[0] * 100).T.rename(index={0: 'null values (%)'})
# display(col_info)
# col_info = pd.concat(col_info, temp)
# display(col_info)
# display(df_init[:5])
...
# temp1 = pd.DataFrame(df_init.isnull().sum()).T.rename(index={0: 'null values (nb)'})
# col_info = pd.concat(col_info, temp1)
#
# temp2 = pd.DataFrame(df_init.isnull().sum() / df_init.shape[0] * 100).T.rename(index={0: 'null values (%)'})
# col_info = pd.concat(col_info, temp2)
