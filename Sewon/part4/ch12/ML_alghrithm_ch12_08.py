...
"""
ratings = pd.read_csv("C:/Users/sewon/Documents/da_study/Sewon/part4/ch12/ratings_small.csv")
ratings = ratings[["userId", "movieId", "rating"]]

ratings.head() 

movies = pd.read_csv("C:/Users/sewon/Documents/da_study/Sewon/part4/ch12/movies_metadata.csv")
links = pd.read_csv("C:/Users/sewon/Documents/da_study/Sewon/part4/ch12/links_small.csv")
...
"""
pivot`함수를 이용해 유저 아이디가 인덱스이고, 영화 아이디가 컬럼, 값이 평가 점수인 
user_movie_matrix 생성
"""
...
user_movie_matrix = user_movie_matrix.fillna(0) #유저가 평가하지 않은 영화에 대해서 결측값을 0으로 대체
user_movie_matrix.shape

#2. KNN Basic
"k가 5인 KNN Basic을 이용해 유저 124가 아직 평가하지 않은 영화 648에 대한 점수를 예측"
k = 5
user_i = 124
movie_id = 648
...
user_i_similarity = user_similarity.loc[user_i] #유저 "124"와 유사한 다른 유저 k명을 찾기
user_i_similarity
...
top_k_weight = (tok_k_similar_ratings > 0) * top_k_similarity #평가 점수가 있는 유저에 대한 Weight 추출
top_k_weight
 
weighted_rating = top_k_weighted_ratings.sum() #유사도가 곱해진 평가 점수의 합을 유사도 합으로 나누기
...
prediction_rating #weight가 0보다 작은 경우 유저 모두 평가하지 않은 경우
...
prediction = pd.Series(prediction_dict).sort_values(ascending=False) #영화 아이디별 예측 평가 점수를 내림차순으로 정렬
...
#3. KNN with Means
"k가 5인 KNN Basic을 이용해 유저124 가 아직 평가하지 않은 영화31에 대한 점수를 예측하는 과정"
user_id = 124
k = 5
movie_i = 648

"""
pivot함수를 이용해 영화 아이디가 인덱스이고, 유저 아이디가 컬럼, 값이 평가 점수인 movie_user_matrix를 생성
결측값은 0으로 대체
"""
...
"영화 648과 유사한 다른 영화 k개를 찾기"
movie_i_similarity = movie_similarity.loc[movie_i] #movie_i와 다른 영화 간의 유사도 추출

movie_i_similarity = movie_i_similarity.sort_values(ascending=False) #다른 영화와의 유사도 내림차순 정렬
...
top_k_weight = (pd.notna(tok_k_similar_ratings)) * top_k_similarity #추출된 영화중 평가 점수가 있는 영화에 대한 가중치만 남김
top_k_weight
...
bias = movie_bias.loc[movie_i] #영화 평균 평점 추출
bias
...
recommend = prediction[:10].index #예측 평가 점수 상위 10개의 영화 아이디 추출
