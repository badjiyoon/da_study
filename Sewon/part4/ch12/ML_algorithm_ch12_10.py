#CH12_10. Matrix Factorization 실습
...
"다른 두 데이터를 이용해 `ratings` 데이터의 `movieId`에 맞는 영화 제목 생성"
...
"""
pivot`함수를 이용해 유저 아이디가 인덱스이고, 
영화 아이디가 컬럼, 값이 평가 점수인 `user_movie_matrix`를 생성
결측값은 0으로 대체
"""
...
"유저 670이 영화 0에 평가한 점수를 학습하는 과정을 소개"
user_id = 670
item_id = 0
...
pred = P[user_id, :].dot(Q[item_id, :].T)
error = real - pred
error #업데이트된 P와 Q를 이용해 오차가 감소했음을 알 수 있음
